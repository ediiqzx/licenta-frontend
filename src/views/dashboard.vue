<template>
								...
								<template v-if="item.composed && viewEntry.content[item.name] != null">
									...
									<!-- <br><br><br>{{ item }} -->
									 <template v-if="item.name == 'id'">
										...
										<asField background="#F7F7F7" type="text" :name="item.name" :label="item.label" :modelValue="viewEntry.content[item.name]" placeholder="No data here" :required="item.required" disabled editable @clicked="toggleEditField(item.name, item.label, item.type)"/>
									</template><template v-else-if="item.type == 'email'">
										...
										<asField background="#F7F7F7" type="date" :name="item.name" :label="item.label" :modelValue="viewEntry.content[item.name]" placeholder="No data here" :required="item.required" disabled editable @clicked="toggleEditField(item.name, item.label, item.type)"/>
									</template><template v-else-if="item.type == 'dropdown'">
										<asField background="#F7F7F7" type="text" :name="item.name" :label="item.label" :modelValue="viewEntry.content[item.name]" placeholder="No data here" :required="item.required" disabled editable @clicked="toggleEditField(item.name, item.label, item.type)"/>
									</template><template v-else-if="item.type == 'currency'">
					...
					<div v-if="viewEntry.structure[viewTable.name].mainRelation && active_workspace.permissions.tables_access[viewEntry.structure[viewTable.name].mainRelation.table + 's'] < 4" class="sectionBox boxtype2">
						<div class="sectionBoxHead">
							...
							<template v-if="active_workspace.permissions.tables_access[viewTable.name] < 4">
								...
								<asButton bttnType="third" :label="'Change ' + viewEntry.structure[viewTable.name].mainRelation.table" icon="edit-25gray-16.png" @click="toggleEditField(viewEntry.structure[viewTable.name].singular + '_' + viewEntry.structure[viewTable.name].mainRelation.table, viewEntry.structure[viewTable.name].singular + ' ' + viewEntry.structure[viewTable.name].mainRelation.table, 'relation')"/>
							</template>
						</div>
						<div class="as-separator"></div>
						<div class="entryFields">
	...
	<div class="modal-overlay" id="editField" v-if="editField.visible">
		<div class="modal-container">
			<div class="modal-head">
				<h3>Edit Field</h3>
				<img src="../assets/close-main-20.png" @click="toggleEditField('close')"/>
			</div>
			<div class="as-separator"></div>
			<form v-on:submit.prevent="submitEditField()" :class="{ 'loading' : editField.loading }">
				<template v-if="editField.type == 'text'">
					<asField :label="editField.label" type="text" name="editFieldInput" :modelValue="editField.value" placeholder="New value" required/>
				</template><template v-else-if="editField.type == 'number' || editField.type == 'currency'">
					<asField :label="editField.label" type="number" name="editFieldInput" :modelValue="editField.value" placeholder="New value" required/>
				</template><template v-else-if="editField.type == 'date'">
					<asField :label="editField.label" type="date" name="editFieldInput" :modelValue="editField.value" placeholder="New value" required/>
				</template><template v-else-if="editField.type == 'dropdown'">
					<div class="asField">
						<label>{{ editField.label }}<span style="color: var(--accent)"> *</span></label>
						<select name="editFieldInput" v-model="editField.value">
							<option disabled value="">Select new value</option>
							<template v-for="item in newEntry.structure[viewTable.name]">
								<template v-if="item.name == editField.name">
									<option v-for="option in item.options" :value="option">{{ option }}</option>
								</template>
							</template>
						</select>
					</div>
				</template><template v-else-if="editField.type == 'toggle'">
					<div class="asField">
						<label>{{ editField.label }}<span style="color: var(--accent)"> *</span></label>
						<select name="editFieldInput" v-model="editField.value">
							<option disabled value="">Select new value</option>
				...
				</template><template v-else-if="editField.type == 'relation'">
					<div class="asField">
						<label>{{ editField.label }}<span style="color: var(--accent)"> *</span></label>
						<select name="editFieldInput" v-model="editField.value">
							<option disabled value="">Select new value</option>
							<option v-for="item in editField.relationOptions" :value="item.id">{{ item.name }}</option>
						</select>
					</div>
				</template>
				<div class="as-separator"></div>
				<div class="formEnd">
					<template v-if="editField.loading == false">
						<asButton bttnType="secondary" label="Cancel" icon="block-white-16.png" @click="toggleEditField('close')"/>
						<asButton bttnType="main" label="Update field" icon="save-white-16.png"/>
					</template>
					<asButton v-else bttnType="loading" label="Please wait..." icon="loading.gif" disabled/>
				</div>
			</form>
...
export default {
	...
	methods: {
		...
		async toggleEditField(fieldName, fieldLabel, fieldType, relationID){
			console.log("Method: toggleEditField(" + fieldName + ", " + fieldLabel + ", " + fieldType + ", " + relationID + ")")

			if (fieldName == "close") this.editField.visible = false
			else {
				// Actualizare view
				this.editField.visible = true
				this.editField.name = fieldName
				this.editField.label = fieldLabel
				this.editField.type = fieldType
				this.editField.relationOptions = []
				if (this.editField.type == 'toggle' && document.querySelector("[name='" + fieldName + "']").value == 'Yes') this.editField.value = true
				else if (this.editField.type == 'toggle' && document.querySelector("[name='" + fieldName + "']").value == 'No') this.editField.value = false
				else if (this.editField.type == 'currency') this.editField.value = document.querySelector("[name='" + fieldName + "']").value.slice(0, -4)
				else if (this.editField.type == 'relation') {
					let relationTable = {
						Contract_client:	{ table: "clients",	field: "client_name" },
					...
					await axios.get(this.apiURL + 'table-' + relationTable[fieldName].table + '/?populate=*&filters[workspace][id][$eq]=' + this.active_workspace.id, { headers: { Authorization: 'Bearer ' + this.user.jwt } } ).then(async (response) => {
						...
						for (let index in response.data.data){
							let item = response.data.data[index]
							let thename = null
							if (relationTable[fieldName].field == "id") thename = item.id
							else thename = item.attributes[relationTable[fieldName].field]
							this.editField.relationOptions.push({
								id: item.id,
								name: thename
							})
						}

						this.editField.value = this.viewEntry.content[fieldName.toLowerCase()].data.id
					}).catch((error) => {
						console.log("error: ", error)
						console.log("error.response: ", error.response)
				...
				} else this.editField.value = document.querySelector("[name='" + fieldName + "']").value
			}
		},
		async submitEditField(){
			console.log("Method: submitEditField()")

			// Declarare requestData
			let requestData = { data: {} }
			requestData.data[this.editField.name.toLowerCase()] = document.querySelector("[name='editFieldInput']").value

			// Actualizare view
			this.editField.loading = true
			
			// Actualizare field
			await axios.put(this.apiURL + 'table-' + this.viewTable.name + '/' + this.viewEntry.id, requestData, { headers: { Authorization: 'Bearer ' + this.user.jwt } } ).then((response) => {
				console.log("response: ", response)
